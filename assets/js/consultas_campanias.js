var listaCampSelOrg = document.getElementById('list-camp-sel-org'),
	listaCampSelDst = document.getElementById('list-camp-sel-dst'),
	listaCampDatosSel = document.getElementById('list-camp-datos-sel'),

	listaDptosSelOrg = document.getElementById('list-dptos-sel-org'),
...
function solicitarCampanias() {
	fetch('/iiet/campanias/listado')
		.then(respuesta => {
			if(respuesta.ok) {
				respuesta.json().then(campanias => rellenarLista(listaCampSelOrg, campanias, 'nombre', 'numero', true));
			}
		});
}
...
document.getElementById('btn-sel-campania').addEventListener('click', e => seleccionarUnItem(listaCampSelOrg, listaCampSelDst));
document.getElementById('btn-sel-campania-todo').addEventListener('click', e => seleccionarItemsTodos(listaCampSelOrg, listaCampSelDst));
document.getElementById('btn-desel-campania').addEventListener('click', e => deseleccionarUnItem(listaCampSelDst));
document.getElementById('btn-desel-campania-todo').addEventListener('click', e => deseleccionarItemsTodos(listaCampSelDst));
...
document.getElementById('btn-ok-sel-campanias').addEventListener('click', function(e) {
	seleccionarItemsTodos(listaCampSelDst, listaCampDatosSel);
	$('#consult-selec-campania').modal('hide');
});
...
document.formCamposConsulta.addEventListener('submit', function(e) {
	this.campanias.value     = JSON.stringify(formatearDatosLista(listaCampDatosSel));
