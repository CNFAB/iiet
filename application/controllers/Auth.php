...
class Auth extends CI_Controller
{
	...
	public function index()
	{
		...
		else
		{
			...
			$this->load->view('auth/index', $datos_usuario);
		}
	}
	...
	public function usuarios() {
		...
		foreach ($this->data['users'] as $k => $user)
		{
			...
			$usuarios[] = array(
				...
				'apellidos' => $this->data['users'][$k]->last_name,
				'nombres' => $this->data['users'][$k]->first_name,
				'email' => $this->data['users'][$k]->email,
				'estado' => $this->data['users'][$k]->active,
				'grupos' => $this->data['users'][$k]->groups
			);
		}

		$json = json_encode(array('data' => $usuarios));
...
public function login($por_ajax = FALSE)
{
    ...
    // Si es una petición AJAX (con /TRUE al final)
    if ($por_ajax) {
	...
	public function forgot_password() {
		...
		else
		{
			...
			if ($forgotten)
			{
				...
				//$this->session->set_flashdata('message', $this->ion_auth->messages());
				$this->load->view('auth/envio_mail_recup_clave'); //we should display a confirmation page here instead of the login page
			}
			else
			{
				//$this->session->set_flashdata('message', $this->ion_auth->errors());
	...
	*/
	...
	public function cambio_clave() {
		$this->load->view('auth/cambio_clave');
	}
